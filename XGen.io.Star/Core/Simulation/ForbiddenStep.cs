...
{
	public class ForbiddenStep<T,TYpe> : Step<T> where T : Core.Type
	{
		protected bool _reached;
		
		public ForbiddenStep(State from, Action action, State to, T results) : base(from, action, to, results)
		{
		}

        public ForbiddenStep(string id, State from, Action action, State to, T results):this(from, action, to, results)
        {
            SetId(id);
        }
		...
		public bool IsForbidden(Step<T> s) 
		{
			if (this._from.Equals(s._from) && this.To.Equals(s.To)) 
			{
				...
				if (strict) 
				{
					if (this.Action.Equals(s.Action)) 
					{
						...
						if (strictest) 
						{
							if (this._targetView.ViewEquals(s._targetView)) 
								return SetReached();
						} 
						else
							return SetReached();
